...
class TSR_Dataset(torch.utils.data.Dataset):
    def __init__(self, x_low1, x_low2, y_train1, y_train2, y_label):
    ...
    def __getitem__(self, idx):
        ...
        return np.array(x_low1), np.array(x_low2), np.array(y_train1), np.array(y_train2), np.array(y_label)
...
class TSR_Net(nn.Module):
    ...
    def forward(self, x1, x2):
        proj1, y1 = self.forward_once(x1)
        proj2, y2 = self.forward_once(x2)
        return proj1, y1, proj2, y2
...
train_set = TSR_Dataset(XTrain_l1, XTrain_l2, YTrain1, YTrain2, YTrain_label)
num_epochs = 30
batch_size = 16
...
for epoch in range(num_epochs):
    ...
    for batch_idex, (data_low1, data_low2, y_train1, y_train2, label) in enumerate(train_loader):
        ...
        p1, cls1, p2, cls2 = model(data_low1, data_low2)

        loss1 = loss_function1(p1, p2, label)       # ContrastiveLoss
        loss3 = loss_function3(cls1, y_train1)       # CrossEntropyLoss
        loss = 1*loss1 + 9*loss3
...
model.eval()
with torch.no_grad():
    p1, cls1, p2, cls2 = model(Xa, Xb)
    Pred_test = cls1.squeeze().cpu().detach().numpy()
    p3, cls3, p4, cls4 = model(Xc, Xd)
    Pred_train = cls3.squeeze().cpu().detach().numpy()
